...
class Two_process:
    ...
    def infer_initial_state(self, sc, rested, t_s, t_w):
        """
        Walk *backwards* through the diary until the very first timestamp,
        starting from the fully-rested (s_w, d_w) at wake - sc[rested].
        """
        
        s1 = self._periodic_values(t_s, t_w)
    ...
    def compute(
        ...
        # ----- initialisations -----
        wake = True
        s0 = self.infer_initial_state(sc, rested, t_s, t_w)
    ...
    # ──────────────────────────────────────────────────────────────
    @staticmethod
    def _periodic_values(t_s, t_w):
        """Compute the periodic values for homeostasis."""
        a = torch.exp(-1 / t_w)
        b = torch.exp(-1 / t_s)

        s_w = (b * (1 - a)) / (1 - a * b) # A revoir complètement suivant ce que je comprends du calcul
        return 0.07
        #return s_w
